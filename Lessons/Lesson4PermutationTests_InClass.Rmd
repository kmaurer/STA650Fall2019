...
Run a t-test with the two samples to compare the means. Must assume: Y is numeric, $\bar{Y}$ is Normal by CLT. 
...
# could calculate the difference in sleep averages
...
# Calculate the difference in sleep averages for all permuations
perm_diffs <- apply(X=sleep_sets, MARGIN = 2, sleep_compare)

# how are the permuted difference distributed?
# histogram is centered at 0 and symmetric
hist(perm_diffs)
# quantiles show same shape
quantile(perm_diffs, seq(.1,.975,by=.05))

# calculate a p-value for Hypothesis Test for difference in mean
# one tail 
real_diff <- with(sleep, mean(extra[group==1]) - mean(extra[group==2]))
...
quantile(real_estate$price, .9)
with(real_estate, quantile(price[pool=="yes"], .9))
with(real_estate, quantile(price[pool=="no"], .9))
with(real_estate, quantile(price[pool=="yes"], .9)-quantile(price[pool=="no"], .9))

#Make a function that randomly permutes the pool labels then compares quantiles
compare_highend_perm <- function(df){
  df$pool <- sample(df$pool, size=length(df$pool))
  with(df, quantile(price[pool=="yes"], .9)-quantile(price[pool=="no"], .9))
}
...
pool_diffs <- rep(NA,M)
for( i in 1:M){
  pool_diffs[i] <- compare_highend_perm(real_estate)
}
head(pool_diffs)

# alternatively could use sapply
pool_diffs2 <- sapply(1:M, function(x) compare_highend_perm(real_estate) )

# p-value for test of difference in 90th percentile prices for homes with and without pools
real_diff <- with(real_estate, quantile(price[pool=="yes"], .9)-quantile(price[pool=="no"], .9))
