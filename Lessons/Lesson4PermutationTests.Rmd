...
sleep_sets <- combn(1:20, 10,simplify = TRUE)
dim(sleep_sets)
sleep_sets[,1:5]

# need a function that can take a vector of indeces, and return the average difference in sleep in the two groups

sleep_diff <- function(idx){
  with(sleep, mean(extra[idx])-mean(extra[-idx]))
}

perm_sleep_diffs <- apply(sleep_sets, 2, sleep_diff)

# Critical values in test
quantile(perm_sleep_diffs,c(0.025,.975))

# permutation test p-values
mean(abs(perm_sleep_diffs) >= abs(sleep_diff(1:10)))
```
...
q90diff <- with(real_estate, quantile(price[pool=="yes"],.9)-quantile(price[pool=="no"],.9))

#Make a function that randomly permutes the pool labels then compares quantiles
perm_pool <- function(df){
  df$pool <- df$pool[sample(1:length(df$pool),length(df$pool))]
  with(df, quantile(price[pool=="yes"],.9)-quantile(price[pool=="no"],.9))
}
perm_pool(real_estate)

# Outcome is random, run it through 10000 MC simulations, could use for loop or rapply()
M=25000

timer <- Sys.time()
perm_diffs <- rep(NA,M)
for(i in 1:M){
  perm_diffs[i] <- perm_pool(real_estate)
}
Sys.time() - timer 

# MC permuation p-value
mean(abs(perm_diffs) >= abs(q90diff))
```
